...
export type Activity = {
  ...
  url?: string;
  /**
   * Starts a fresh column instead of flowing on. Only honoured where the list
   * actually has three columns — the entry would otherwise be stranded alone in
   * column two on a narrower screen.
   */
  breakBefore?: boolean;
...
export const activities: Activity[] = [
  ...
  },
  {
    at: 20260818,
    period: "2026.08.18",
    title: "스타트업을 위한 AWS 기반 AI 에이전트 프로덕션 배포와 비용·성능 최적화 전략",
    highlight: true,
    breakBefore: true,
    detail: {
      org: "AWS · 스타트업 대상 세션",
      facts: [
        { label: "형태", value: "세미나 · 참석" },
        { label: "일자", value: "2026.08.18" },
        { label: "주제", value: "AI 에이전트 프로덕션 배포 · 비용 · 성능" },
      ],
      summary:
        "에이전트를 ==데모까지 만드는 일==과 ==프로덕션에서 굴리는 일==은 다른 문제다. 데모는 " +
        "한 사람이 한 번 부르지만, 서비스는 여러 사람이 동시에 부르고 그만큼이 그대로 " +
        "청구서와 응답 시간으로 돌아온다. 이 세션은 그 간격 — ==비용==, ==지연==, ==운영 " +
        "안정성== — 을 스타트업 규모에서 어떻게 감당할지를 다뤘다.",
      sections: [
        {
          heading: "데모와 프로덕션 사이",
          body: [
            "에이전트는 잘 도는 걸 보여주기는 쉽다. 툴을 몇 개 붙이고 프롬프트를 다듬으면 " +
              "혼자 쓰기에는 충분히 그럴듯하게 움직인다. 문제는 그다음이다.",
            "사용자가 늘면 모델 호출 수가 그대로 따라 늘고, 호출당 토큰이 곧 비용이다. " +
              "인프라 비용처럼 계단식으로 오르지 않고 사용량에 선형으로 붙기 때문에, " +
              "설계 단계에서 잡아두지 않으면 나중에 줄일 수 있는 여지가 거의 없다.",
          ],
        },
        {
          heading: "비용 — 토큰이 곧 청구서",
          body: [
            "비용을 줄이는 방법은 결국 '덜 보내고, 다시 안 보내고, 큰 모델을 덜 쓰는' " +
              "세 가지로 모인다.",
          ],
          list: [
            "반복되는 앞부분(시스템 프롬프트 · 지침 · 참조 문서)은 캐싱해 매 호출마다 다시 태우지 않는다",
            "컨텍스트는 넣을 수 있는 만큼이 아니라 필요한 만큼만 — 대화 이력과 검색 결과를 그대로 흘려보내지 않는다",
            "모든 단계에 같은 모델을 쓰지 않는다. 분류 · 라우팅 같은 가벼운 판단은 작은 모델로 내리고, 어려운 추론에만 큰 모델을 부른다",
            "실시간이 아니어도 되는 작업은 배치로 미룬다 — 사용자가 기다리지 않는 일에 실시간 단가를 낼 이유가 없다",
          ],
        },
        {
          heading: "성능 — 기다리는 시간을 줄이는 쪽",
          body: [
            "체감 속도는 총 처리 시간이 아니라 '첫 글자가 언제 나오는가'로 결정된다. " +
              "스트리밍으로 응답을 흘리고, 서로 의존하지 않는 툴 호출은 병렬로 던지고, " +
              "긴 작업은 요청을 붙잡는 대신 비동기로 떼어내 상태만 돌려준다.",
            "여기서 나오는 결론이 결국 큐와 배치다. 요청을 받는 자리와 실제로 일하는 " +
              "자리를 분리해두면, 몰리는 시간대에도 앞단이 먼저 무너지지 않는다.",
          ],
        },
        {
          heading: "지금 하는 일에 대입해보면",
          body: [
            "인앤써의 AI 면접 문제 생성도 같은 경로를 지나왔다. 동기 처리라 요청 하나가 " +
              "생성이 끝날 때까지 자리를 붙잡고 있었고, 사람이 몰리면 그대로 밀렸다. " +
              "이걸 비동기 배치로 바꾸면서 시간과 실패율이 함께 내려갔다.",
            "그래서 이 세션의 내용이 새로운 이야기라기보다, 겪어서 알게 된 것을 " +
              "정리된 형태로 다시 본 쪽에 가깝다. 남은 숙제는 비용 쪽이다 — 지연은 " +
              "구조로 잡았지만, 토큰은 아직 붙는 만큼 나가고 있다.",
          ],
        },
      ],
    },
